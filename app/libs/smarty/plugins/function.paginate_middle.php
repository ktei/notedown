...
function smarty_function_paginate_middle($params, &$smarty) {
    ...
    $_prefix = '[';
    $_suffix = ']';
    $_link_prefix = '<li>'; // for twitter bootstrap style
    $_link_suffix = '</li>'; // for twitter bootstrap style
    $_page_limit = 5;
    $_attrs = array();
    ...
    while ($_item <= $_total) {
        ...
        if ($_item != $_curr_item) {
            ...
            // Made this change to apply twitter bootstrap style
            $_ret .= '<li class="active"><span>' . $_text . '</span></li>';
        }
        $_item += $_limit;
        $_page++;
        $_display_pages++;
        if (isset($_page_limit) && $_display_pages == $_page_limit + 1)
            break;
    }
